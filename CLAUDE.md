...
- **Receipt OCR runs entirely in the browser** — no API, no key, no per-scan cost, and the photo never leaves the device. Two modules, deliberately split:
  - `src/lib/ocr.ts` (client-only) preprocesses the photo on a canvas (scale to ~2000px → grayscale → **Sauvola** local adaptive threshold) and runs Tesseract.js WASM against it, returning raw text. Tesseract assets (~5 MB) load on demand from the CDN on first scan and are then cached in IndexedDB.
    - Preprocessing is not optional: measured on three real receipts, skipping it drops price recovery from 27/30 to 17/30. Sauvola beat global Otsu (27/30 vs 26/30, and fewer spurious items) because a crumpled receipt is lit unevenly and one global threshold blows out half of it.
    - Page segmentation mode 4 (single column) matters just as much — PSM 6 collapsed the same set to 11-15/30. Adding `eng` alongside `ita` changed nothing, so it isn't worth the extra download.
