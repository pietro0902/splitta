...
- **Receipt OCR runs entirely in the browser** — no API, no key, no per-scan cost, and the photo never leaves the device. Two modules, deliberately split:
  - `src/lib/ocr.ts` (client-only) owns the scan. `scanReceipt()` preprocesses the photo on a canvas (scale → grayscale → **Sauvola** local adaptive threshold) and runs Tesseract.js WASM against it, then parses and reconciles. Tesseract assets (~5 MB) load on demand from the CDN on first scan and are then cached in IndexedDB.
    - **It runs up to three passes with different settings** (2000px/k=0.2, 2000px/k=0.1, 2600px/k=0.2), stopping early as soon as one reconciles against the printed total. This is the single biggest quality lever found: measured on four real receipts, the best single pass got 31/34 prices and 2/4 totals, while reconciling the passes got **34/34 and 4/4**. No single configuration won on all four — one photo went 17/18, 15/18, 18/18 across the three.
